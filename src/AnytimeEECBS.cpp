  ...
  while (runtime < time_limit && sum_of_costs > sum_of_costs_lowerbound) {
    ...
    if (ecbs.solution_found) {
      if (sum_of_costs > ecbs.solution_cost) {
        ...
        for (int i = 0; i < num_of_agents; i++) solution[i] = *ecbs.paths[i];
      }
      ...
      iteration_stats.emplace_back(instance.getDefaultNumberOfAgents(), sum_of_costs, runtime,
                                   "EECBS(" + std::to_string(w) + ")", sum_of_costs_lowerbound);
    }
