  ...
  switch (type) {
    ...
    case constraint_type::POSITIVE_EDGE:
      ...
      if (agent == a)  // this agent has to be at x at timestep t - 1 and be at y at timestep t
      {
        ...
        insertLandmark(y, t);
      } else  // other agents cannot stay at x at timestep t - 1, be at y at timestep t, or traverse edge (y, x) from
              // timesteps t - 1 to t
      {
        insert2CT(x, t - 1, t);
        insert2CT(y, t, t + 1);
...
bool ConstraintTable::constrained(size_t loc, int t) const {
  ...
  if (loc < map_size) {
    ...
    if (it != landmarks.end() && it->second != loc) return true;  // violate the positive vertex constraint
  }
...
int ConstraintTable::getNumOfConflictsForStep(size_t curr_id, size_t next_id, int next_timestep) const {
  int rst = 0;
  if (path_table_for_CAT != nullptr) rst = path_table_for_CAT->getNumOfCollisions(curr_id, next_id, next_timestep);
...
bool ConstraintTable::hasConflictForStep(size_t curr_id, size_t next_id, int next_timestep) const {
  if (path_table_for_CAT != nullptr and path_table_for_CAT->hasCollisions(curr_id, next_id, next_timestep)) return true;
  if (!cat.empty()) {
...
bool ConstraintTable::hasEdgeConflict(size_t curr_id, size_t next_id, int next_timestep) const {
  ...
  return !cat.empty() and curr_id != next_id and cat[next_id].size() >= next_timestep and
         cat[curr_id].size() > next_timestep and cat[next_id][next_timestep - 1] and cat[curr_id][next_timestep];
}
...
int ConstraintTable::getHoldingTime(int location, int earliest_timestep) const {
  ...
  if (path_table_for_CT != nullptr) rst = path_table_for_CT->getHoldingTime(location, earliest_timestep);
  // CT
