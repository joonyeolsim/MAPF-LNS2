  ...
  for (auto it = sit[location].begin(); it != sit[location].end(); ++it) {
    ...
    if (i_min < t_min && i_max <= t_max) {
      if (it != sit[location].end() and std::next(it) != sit[location].end() and
          (location != goal_location || i_max != constraint_table.length_min) and i_max == get<0>(*std::next(it)) and
...
void ReservationTable::updateSIT(int location) {
  ...
  if (location == goal_location)  // we need to divide the same intervals into 2 parts [0, length_min) and [length_min,
                                  // length_max + 1)
  {
  ...
  // soft path table
  if (constraint_table.path_table_for_CAT != nullptr and !constraint_table.path_table_for_CAT->table.empty()) {
    if (location < constraint_table.map_size)  // vertex conflict
    {
...
list<tuple<int, int, int, bool, bool>> ReservationTable::get_safe_intervals(int from, int to, int lower_bound,
                                                                            int upper_bound) {
  ...
  for (auto interval : sit[to]) {
    ...
    auto t1 =
        get_earliest_arrival_time(from, to, max(lower_bound, get<0>(interval)), min(upper_bound, get<1>(interval)));
