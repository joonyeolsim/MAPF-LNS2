...
Path SIPP::findPath(const ConstraintTable& constraint_table) {
  ...
  while (!focal_list.empty()) {
    ...
    {
      for (auto& i : reservation_table.get_safe_intervals(curr->location, next_location, curr->timestep + 1,
        ...
        auto next = new SIPPNode(next_location, next_timestep, next_h_val, curr, next_timestep, next_high_generation,
    ...
    if (curr->high_expansion == curr->high_generation and
      ...
      auto next_collisions = curr->num_of_conflicts +
                             ...
                             (int)get<2>(interval);
      auto next = new SIPPNode(curr->location, next_timestep, next_h_val, curr, next_timestep, get<1>(interval),
                               get<1>(interval), get<2>(interval), next_collisions);
...
Path SIPP::findOptimalPath(const HLNode& node, const ConstraintTable& initial_constraints, const vector<Path*>& paths,
                           int agent, int lowerbound) {
    ...
    for (int next_location : instance.getNeighbors(curr->location))  // move to neighboring locations
    {
      for (auto& i : reservation_table.get_safe_intervals(curr->location, next_location, curr->timestep + 1,
        ...
        int next_conflicts = curr->num_of_conflicts +
                             //(int)curr->collision_v * max(next_timestep - curr->timestep - 1, 0) +
                             (int)next_v_collision + (int)next_e_collision;
        auto next = new SIPPNode(next_location, next_g_val, next_h_val, curr, next_timestep, next_high_generation,
    ...
    for (int next_location : next_locations) {
      ...
      if (!constraint_table.constrained(next_location, next_timestep) &&
        ...
        if (next_g_val + next_h_val >= upper_bound)  // the cost of the path is larger than the upper bound
          continue;
        auto next = new SIPPNode(next_location, next_g_val, next_h_val, nullptr, next_timestep, next_timestep + 1,
                                 next_timestep + 1, 0, 0);
        if (dominanceCheck(next))
          pushNodeToOpenAndFocal(next);
        else
