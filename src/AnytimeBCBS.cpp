...
void AnytimeBCBS::run() {
  ...
  double w = (double)MAX_COST / (instance.num_of_cols + instance.num_of_rows) / 10 /
             num_of_agents;  // a large enough w without integer overflow issue
  ...
  while (runtime < time_limit && sum_of_costs > sum_of_costs_lowerbound) {
    ...
    if (bcbs.solution_found) {
      ...
      iteration_stats.emplace_back(instance.getDefaultNumberOfAgents(), sum_of_costs, runtime, "BCBS",
                                   sum_of_costs_lowerbound);
    }
  ...
  if (best_goal_node == nullptr)
    ...
    for (int i = 0; i < num_of_agents; i++) solution[i] = *bcbs.paths[i];
  }
  ...
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j < N; j++) {
      ...
      for (; t < (int)solution[a1].size(); t++) {
        ...
        {
          cerr << "Find a vertex conflict between agents " << a1 << " and " << a2 << " at location "
               << solution[a1][t].location << " at timestep " << t << endl;
          exit(-1);
                   ...
                   solution[a1][t - 1].location == solution[a2][t].location)  // edge conflict
        {
          cerr << "Find an edge conflict between agents " << a1 << " and " << a2 << " at edge ("
               << solution[a1][t - 1].location << "," << solution[a1][t].location << ") at timestep " << t << endl;
          exit(-1);
        }
      }
      ...
      for (; t < (int)solution[a2].size(); t++) {
        ...
        {
          cerr << "Find a target conflict where agent " << a2 << " traverses agent " << a1 << "'s target location "
...
void AnytimeBCBS::writeIterStatsToFile(string file_name) const {
  ...
  for (const auto& data : iteration_stats) {
    output << data.num_of_agents << "," << data.sum_of_costs << "," << data.runtime << ","
           << data.sum_of_costs_lowerbound << "," << data.algorithm << endl;
  }
  output.close();
}
...
void AnytimeBCBS::writeResultToFile(string file_name) const {
  ...
  stats << runtime << "," << sum_of_costs << "," << iteration_stats.front().sum_of_costs << ","
        << sum_of_costs_lowerbound << "," << sum_of_distances << "," << iteration_stats.size() << ","
        << iteration_stats.front().runtime << "," << auc << "," << preprocessing_time << "," << getSolverName() << ","
