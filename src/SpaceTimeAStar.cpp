...
Path SpaceTimeAStar::findPath(const ConstraintTable& constraint_table) {
  ...
  auto holding_time = constraint_table.getHoldingTime(
      goal_location, constraint_table.length_min);  // the earliest timestep that the agent can hold its goal location.
                                                    // The length_min is considered here.
  ...
  while (!focal_list.empty()) {
               ...
               curr->timestep >= holding_time)      // the agent can hold the goal location afterward
    {
      ...
      } else  // update existing node's if needed (only in the open_list)
      {
        ...
        if (existing_next->num_of_conflicts > goal->num_of_conflicts ||
            (existing_next->num_of_conflicts == goal->num_of_conflicts && existing_next->getFVal() > goal->getFVal())) {
    ...
    for (int next_location : next_locations) {
      ...
      auto next = new AStarNode(next_location, next_g_val, next_h_val, curr, next_timestep, num_conflicts);

      if (next_location == goal_location && curr->location == goal_location) next->wait_at_goal = true;
      ...
      if (existing_next->num_of_conflicts > next->num_of_conflicts ||
          (existing_next->num_of_conflicts == next->num_of_conflicts && existing_next->getFVal() > next->getFVal())) {
...
pair<Path, int> SpaceTimeAStar::findSuboptimalPath(const HLNode& node, const ConstraintTable& initial_constraints,
  ...
  while (!open_list.empty()) {
    ...
    for (int next_location : next_locations) {
      ...
      if (next_g_val + next_h_val > constraint_table.length_max) continue;
      int next_internal_conflicts = curr->num_of_conflicts + constraint_table.getNumOfConflictsForStep(
                                                                 curr->location, next_location, next_timestep);

      // generate (maybe temporary) node
      auto next = new AStarNode(next_location, next_g_val, next_h_val, curr, next_timestep, next_internal_conflicts);
      if (next_location == goal_location && curr->location == goal_location) next->wait_at_goal = true;
               ...
               next->num_of_conflicts))  // or it remains the same but there's fewer conflicts
      {
        if (!existing_next->in_openlist)  // if its in the closed list (reopen)
        {
          ...
          bool add_to_focal =
              false;  // check if it was above the focal bound before and now below (thus need to be inserted)
          bool update_in_focal =
              false;  // check if it was inside the focal and needs to be updated (because f-val changed)
          ...
          if (update_open) open_list.increase(existing_next->open_handle);  // increase because f-val improved
          if (add_to_focal) existing_next->focal_handle = focal_list.push(existing_next);
          if (update_in_focal)
            focal_list.update(
                existing_next
                    ->focal_handle);  // should we do update? yes, because number of conflicts may go up or down
        }
      }
...
int SpaceTimeAStar::getTravelTime(int start, int end, const ConstraintTable& constraint_table, int upper_bound) {
  ...
  while (!open_list.empty()) {
    ...
    for (int next_location : next_locations) {
      ...
      if (!constraint_table.constrained(next_location, next_timestep) &&
          !constraint_table.constrained(curr->location, next_location, next_timestep)) {  // if that grid is not blocked
...
void SpaceTimeAStar::updateFocalList() {
  ...
  if (open_head->getFVal() > min_f_val) {
    ...
    for (auto n : open_list) {
      if (n->getFVal() > w * min_f_val && n->getFVal() <= w * new_min_f_val) n->focal_handle = focal_list.push(n);
    }
