...
bool InitLNS::run() {
  ...
  while (runtime < time_limit and num_of_colliding_pairs > 0) {
    ...
    for (int i = 0; i < (int)neighbor.agents.size(); i++) {
      ...
      if (replan_algo_name == "PP" || neighbor.agents.size() == 1) neighbor.old_paths[i] = agents[a].path;
...
bool InitLNS::runGCBS() {
  ...
  for (int i = 0; i < (int)neighbor.agents.size(); i++) {
    ...
    for (int j = 0; j < instance.getDefaultNumberOfAgents(); j++) {
      if (j != agent_id and collision_graph[agent_id].count(j) == 0) path_tables[i].insertPath(j, agents[j].path);
    }
  }
...
bool InitLNS::generateNeighborByCollisionGraph() {
  ...
  neighbor.agents.assign(neighbors_set.begin(), neighbors_set.end());
  if (screen >= 2) cout << "Generate " << neighbor.agents.size() << " neighbors by collision graph" << endl;
  return true;
}
...
void InitLNS::writeResultToFile(const string& file_name, int sum_of_distances, double preprocessing_time) const {
  ...
  stats << runtime << "," << iteration_stats.back().num_of_colliding_pairs << "," << sum_of_costs << ","
        << iteration_stats.front().num_of_colliding_pairs << "," << iteration_stats.front().sum_of_costs << ","
        << sum_of_distances << "," << iteration_stats.size() << "," << average_group_size << ","
        << iteration_stats.front().runtime << "," << auc << "," << num_LL_expanded << "," << num_LL_generated << ","
        << num_LL_reopened << "," << num_LL_runs << "," << preprocessing_time << "," << getSolverName() << ","
