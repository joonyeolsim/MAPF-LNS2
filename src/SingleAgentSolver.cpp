...
void SingleAgentSolver::findMinimumSetofColldingTargets(vector<int>& goal_table, set<int>& A_target) {
  ...
  while (!open_list.empty()) {
    ...
    if (curr->location == goal_location)  // arrive at the goal location
    {
      ...
      for (auto& p : path) {
        if (goal_table[p.location] > -1 && p.location != goal_location) A_target.insert(goal_table[p.location]);
      }
      break;
    }
    ...
    for (int next_location : next_locations) {
      ...
      if (visited[next_location] == nullptr) {
        ...
        auto next = new Node(next_location, next_g_val, my_heuristic[next_location], num_of_visited_targets, curr);
