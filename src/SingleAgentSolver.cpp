...
void SingleAgentSolver::findMinimumSetofColldingTargets(vector<int>&goal_table, set<int>&A_target) {
    struct Node {
        ...
        Node(int location, int g_val, int h_val, int num_of_targets, Node* parent) : location(location), g_val(g_val),
            h_val(h_val), num_of_targets(num_of_targets), parent(parent) {
        }
