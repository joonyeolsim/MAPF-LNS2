...
class ConstraintTable {
 public:
  ...
  // void clear(){ct.clear(); cat_small.clear(); cat_large.clear(); landmarks.clear(); length_min = 0, length_max =
  // INT_MAX; latest_timestep = 0;}
 ...
 protected:
  ...
  // typedef unordered_map<size_t, set< pair<int, int> > > CAT; // conflict avoidance table // location -> time range,
  // or edge -> time range
  ...
  void insertLandmark(
      size_t loc, int t);  // insert a landmark, i.e., the agent has to be at the given location at the given timestep
